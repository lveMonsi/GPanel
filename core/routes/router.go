	...
	api.Use(middleware.OperationLog())
	{
		v1 := api.Group("/v1")
		{
			...
			agent := v1.Group("/agent")
			{
				...
				agent.POST("/ssh/file", middleware.Auth(), sshController.LoadSSHFile)
				agent.POST("/ssh/file/update", middleware.Auth(), sshController.UpdateSSHFile)
				agent.POST("/ssh/keys/search", middleware.Auth(), sshController.SearchSSHKeys)
				agent.POST("/ssh/keys", middleware.Auth(), sshController.CreateSSHKey)
				agent.POST("/ssh/keys/update", middleware.Auth(), sshController.UpdateSSHKey)
				agent.POST("/ssh/keys/delete", middleware.Auth(), sshController.DeleteSSHKeys)
				agent.POST("/ssh/keys/sync", middleware.Auth(), sshController.SyncSSHKeys)
