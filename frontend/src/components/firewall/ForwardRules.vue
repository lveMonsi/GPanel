  ...
  <div class="forward-rules">
    ...
    <div class="rules-content">
      ...
      >
        ...
        <el-table-column prop="port" label="源端口" min-width="120" />
        <el-table-column prop="targetIP" label="目标IP" min-width="150">
          <template #default="{ row }">
            {{ row.targetIP || '127.0.0.1' }}
          </template>
        </el-table-column>
        <el-table-column prop="targetPort" label="目标端口" min-width="120" />
    ...
    >
      <el-form :model="form" :rules="rulesValidator" label-width="80px" ref="formRef">
        ...
        <el-form-item label="源端口" prop="port">
          <el-input v-model="form.port" placeholder="例如: 80" />
        </el-form-item>
        <el-form-item label="目标IP" prop="targetIP">
          <el-input v-model="form.targetIP" placeholder="留空表示转发到本地" />
        </el-form-item>
        <el-form-item label="目标端口" prop="targetPort">
          <el-input v-model="form.targetPort" placeholder="例如: 8080" />
        </el-form-item>
        <el-form-item label="网络接口" prop="interface">
          <el-input v-model="form.interface" placeholder="留空表示所有接口" />
        </el-form-item>
      </el-form>
      <template #footer>
...
import { searchRules, operateForwardRule } from '@/api/modules/firewall';
import type { FireInfo, ForwardRule } from '@/api/interface/firewall';
...
const rulesValidator = {
  ...
  port: [{ required: true, message: '请输入源端口', trigger: 'blur' }],
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit.value && editRow.value?.num) {
      // 编辑：先删除旧规则，再添加新规则
      await operateForwardRule({
        forceDelete: false,
        rules: [{ ...editRow.value, operation: 'remove' }],
      });
    }
    ...
    await operateForwardRule({
      forceDelete: false,
      rules: [{ ...form.value, operation: 'add' }],
    });
...
const handleBatchDelete = async () => {
  try {
    ...
    await operateForwardRule({
      forceDelete: true,
      rules: selectedRows.value.map(row => ({ ...row, operation: 'remove' })),
    });
