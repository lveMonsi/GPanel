  ...
  <div class="ip-rules">
    ...
    <div class="rules-content">
      ...
      >
        ...
        <el-table-column prop="address" label="IP地址" min-width="180">
    ...
    >
      <el-form :model="form" :rules="rulesValidator" label-width="80px" ref="formRef">
        <el-form-item label="IP地址" prop="address">
          <el-input v-model="form.address" placeholder="例如: 192.168.1.100 或 192.168.1.0/24" :disabled="isEdit" />
        </el-form-item>
        <el-form-item label="协议" prop="protocol">
          <el-select v-model="form.protocol" placeholder="选择协议（留空表示全部）" style="width: 100%">
...
import { searchRules, operateIPRule, updateIPRule } from '@/api/modules/firewall';
import type { FireInfo, IPRuleOperate } from '@/api/interface/firewall';
...
const handleSave = async () => {
  ...
  try {
    ...
    if (isEdit.value && editRow.value) {
      ...
      await updateIPRule({ oldRule, newRule });
    } else {
      // 添加
      await operateIPRule(form.value);
    }
