...
func SetupRouter(r *gin.Engine) {
	...
	api.Use(middleware.Auth())
	{
		v1 := api.Group("/v1")
		{
			...
			ssh := v1.Group("/ssh")
			{
				ssh.GET("/info", sshController.GetSSHInfo)
				ssh.POST("/operate", sshController.OperateSSH)
				ssh.POST("/config", sshController.UpdateSSHConfig)
				ssh.POST("/file", sshController.LoadSSHFile)
				ssh.POST("/file/update", sshController.UpdateSSHFile)
				ssh.POST("/keys/search", sshController.SearchSSHKeys)
				ssh.POST("/keys", sshController.CreateSSHKey)
				ssh.POST("/keys/update", sshController.UpdateSSHKey)
				ssh.POST("/keys/delete", sshController.DeleteSSHKeys)
				ssh.POST("/keys/sync", sshController.SyncSSHKeys)
				ssh.GET("/sessions", sshController.GetSSHSessions)
				ssh.POST("/sessions/kill", sshController.KillSSHSession)
			...
			{
				monitor.POST("/query", monitorController.QueryData)
				monitor.DELETE("/data", monitorController.ClearData)
				monitor.GET("/io-options", monitorController.GetIOOptions)
				monitor.GET("/network-options", monitorController.GetNetworkOptions)
